...
:menu_principal
set "opts=Gestionnaire DNS Cloudflare;Mises a jour des application windows;Menu contextuel Windows 11;Formatage avec DISKPART;Export mots de passe navigateurs;[--- OUTILS AVANCES ---];Voir les outils systeme avances"
...
rem Script PowerShell aéré avec rendu differentiel intelligent, anti-saut et anti-scintillement absolu
set "ps_code=$o='%m_opts%'-split';';$t='%m_title%';$sel=@();for($i=0;$i -lt $o.Count;$i++){if($o[$i] -notmatch '^\[---'){$sel+=$i}};$sIdx=0;$oldIdx=-1;$iY=@{};clear-host;$bY=[console]::CursorTop;function D{ [console]::SetCursorPosition(0,$bY);write-host '                                                                                                                       ';write-host '  ========================================================================================' -f Cyan;write-host ('   ' + $t) -f White;write-host '  ========================================================================================' -f Cyan;write-host '                                                                                                                       ';$num=1;for($i=0;$i -lt $o.Count;$i++){$parts=$o[$i]-split'~';$s=$parts[0];$d='';if($parts.Count -gt 1){$d=$parts[1]};if($s -match '^\[---'){if($i -gt 0){write-host '                                                                                                                       '};$iY[$i]=[console]::CursorTop;write-host ('       ' + $s).PadRight(119) -f Cyan}else{$iY[$i]=[console]::CursorTop;if($i -eq $sel[$sIdx]){$str='    >> [{0}] {1}  ' -f $num, $s; write-host $str -NoNewline -f Black -b White; $rem=119-$str.Length; if($rem -lt 0){$rem=0}; $ds=if($d){'   - '+$d}else{''}; if($ds.Length -gt $rem){$ds=$ds.Substring(0,$rem)}; write-host $ds.PadRight($rem) -f Yellow}else{$str='       [{0}] {1}  ' -f $num, $s; write-host $str.PadRight(119) -f Gray};$num++}};write-host '                                                                                                                       ';write-host '  ----------------------------------------------------------------------------------------' -f Cyan;write-host '   [FLECHES] Naviguer | [ENTREE] Valider | [0/ECHAP] Retour                                       ' -f DarkGray;write-host '                                                                                                                       '};D;while($true){if($oldIdx -ne -1 -and $oldIdx -ne $sIdx){$i=$sel[$oldIdx];$parts=$o[$i]-split'~';$s=$parts[0];$num=$oldIdx+1;[console]::SetCursorPosition(0,$iY[$i]);$str='       [{0}] {1}  ' -f $num, $s; write-host $str.PadRight(119) -f Gray;$i=$sel[$sIdx];$parts=$o[$i]-split'~';$s=$parts[0];$d='';if($parts.Count -gt 1){$d=$parts[1]};$num=$sIdx+1;[console]::SetCursorPosition(0,$iY[$i]);$str='    >> [{0}] {1}  ' -f $num, $s; write-host $str -NoNewline -f Black -b White; $rem=119-$str.Length; if($rem -lt 0){$rem=0}; $ds=if($d){'   - '+$d}else{''}; if($ds.Length -gt $rem){$ds=$ds.Substring(0,$rem)}; write-host $ds.PadRight($rem) -f Yellow};$oldIdx=$sIdx;try{[console]::SetCursorPosition(0,$iY[$o.Count-1]+5)}catch{};$k=$Host.UI.RawUI.ReadKey('NoEcho,IncludeKeyDown');$v=$k.VirtualKeyCode;if($v -eq 38){$sIdx--;if($sIdx -lt 0){$sIdx=$sel.Count-1}}elseif($v -eq 40){$sIdx++;if($sIdx -ge $sel.Count){$sIdx=0}}elseif($v -eq 13){clear-host;exit ($sIdx+1)}elseif($v -eq 27 -or $k.Character -eq '0'){clear-host;exit 0}elseif([string]$k.Character -match '^[1-9]$' -and [int][string]$k.Character -le $sel.Count){clear-host;exit ([int][string]$k.Character)}}"
